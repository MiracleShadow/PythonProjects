...
if __name__=='__main__':
    table = PrettyTable(["标题", "时间", "来源", "作者", "审核", "上传", "摄影", "审定发布","新闻链接"])
    ...
    for inum in range(1,int(input("请输入你想查询的页数:"))+1):
        ...
        for i in range(len(title_list)):
            table.add_row([title_list[i],time_list[i],text_list[0][i],text_list[1][i],text_list[2][i],text_list[3][i],text_list[4][i],text_list[5][i],url_list[i]])
    print(table)